...
static struct fat_filesystem FILESYSTEM = {
    ...
    .root_dir = {{.filename = {'T', 'i', 'n', 'y', 'U', 'S', 'B', ' '}, // name
                 ...
                 .extension = {'T', 'X', 'T'}, .creation_time = {0x52, 0x6D},
                  ...
                  .starting_cluster = {0x02, 0x00}, .file_size = {U32_TO_4U8(sizeof(README_CONTENTS) - 1)}},
                 {
                     .filename = {'F', 'P', 'G', 'A', ' ', ' ', ' ', ' '},
                     .extension = {' ', ' ', ' '}, // extension
...
int32_t get_file_cluster(uint16_t parent_cluster, char *filename)
{
    ...
    if (!parent_cluster) { // parent cluster being root dir is a special case
        parent_dir = FILESYSTEM.root_dir;
        dir_entries = NUM_ROOT_DIR_ENTRIES;
    }
    if (!parent_dir) {
        if (!cluster_to_fat_table_val(parent_cluster)) return -1; // if parent cluster is unallocated, abort
        parent_dir = FILESYSTEM.directories[parent_cluster - 2];
    }
    ...
    if (!cluster_to_fat_table_val(cluster_num)) return -1; // if the file we're looking at is unallocated
    return cluster_num;
}
...
int is_cluster_in_chain(uint16_t starting_cluster, uint16_t ciq)
{
    ...
    while (starting_cluster != 0xFFFF) {
        ...
        starting_cluster = cluster_to_fat_table_val(starting_cluster);
    }
    return 0;
}
...
int get_first_file_in_dir(uint16_t parent_cluster, struct directory_entry *info)
{
    ...
    if (!cluster_to_fat_table_val(folder_cluster)) return -1; // this cluster is free

    struct directory_entry *folderptr = FILESYSTEM.directories[folder_cluster - 2];
